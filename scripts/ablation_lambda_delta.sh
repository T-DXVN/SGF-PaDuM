DELAY_TIME=1
echo "将等待 $DELAY_TIME 秒"
sleep $DELAY_TIME
echo "已等待 $DELAY_TIME 秒"
...
seq_len_meta=96
testname=SGF_ablation   #Newfusion_cnn_mamba_Huberloss_xiuzheng8.0_0.05
testnames=""
pred_lens_meta=(96 192 336 720) #96 192 336 720
epochs=(40 40 40 40) #20 20 20 20  40 40 40 40
patch_lens=(16)
strides_meta=(8)
label_len_mata=48
lradj='type3' #cosine
...
models=("TBD") #"PaDuM" "SGF"
delta_values=(0.01 0.05 0.1 0.5)
lambda_values=(4 8 10 12)

for delta in "${delta_values[@]}"; do
    for lambda in "${lambda_values[@]}"; do
        testnames="${testname}_${delta}_${lambda}"
    

# === 为每个模型 + 测试名 + ma_type + 数据集 创建独立目录 ===
for model_name in "${models[@]}"; do
    for dataset in "${datasets[@]}"; do
        log_dir="./logs/${model_name}/${testnames}/${dataset}"
        if [ ! -d "$log_dir" ]; then
            mkdir -p "$log_dir"
        ...
        for i in "${!pred_lens[@]}"; do
            for j in "${!patch_lens[@]}"; do
                for n in "${!strides[@]}"; do
                    ...
                    # 日志文件名
                    log_file="logs/$model_name/${testnames}/$dataset/${model_name}_${dataset}_${seq_len}_${pred_len}_${current_time}.log"
